...
// - Blender state (Additive blending, gAlpha blending etc.)
...
bool CreateStates()
{
    ...
    //** Despite the word "Alpha" in the variable names, these are not the settings used for gAlpha blending
    ...
    blendDesc.RenderTarget[0].BlendEnable = TRUE;           // Enable blending 
    blendDesc.RenderTarget[0].SrcBlend = D3D11_BLEND_ZERO;   // How to blend the source (texture colour)
    blendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_SRC_COLOR;  // How to blend the destination (colour already on screen)
    blendDesc.RenderTarget[0].BlendOp = D3D11_BLEND_OP_ADD; // How to combine the above two, almost always ADD
